document.addEventListener("DOMContentLoaded", function () {
    ...
    if (urlDest) {
        ...
        var cb = document.querySelector('#filtr-dest-modal .cb-mesto[value="' + urlDest + '"]');
        if (cb) {
            cb.checked = true;
            cb.dispatchEvent(new Event('change', { bubbles: true }));
            
            // Aktualizujeme text na triggeru
            var triggerText = document.getElementById('filtr-dest-trigger-text');
            if (triggerText) {
                triggerText.textContent = cb.getAttribute('data-nazev');
            }
        }
    }
